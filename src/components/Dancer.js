...
import { Group, Line, RegularPolygon, Rect, Arc, Circle, Transformer } from 'react-konva';
...
const Dancer = ({ panelId, id }) => {
  ...
  const isSelected = selectedDancer && selectedDancer.panelId === panelId && selectedDancer.dancerId === id;
  ...
  const handlePartDragEnd = useCallback((part, side) => (e) => {
    const newPos = e.target.position();
    updateDancerState(panelId, id, { [`${side}${part}Pos`]: newPos });
  }, [updateDancerState, panelId, id]);

  // This function handles rotating a hand
  const handleHandRotation = useCallback((e) => {
    const node = e.target;
    const rotation = node.rotation();
    updateDancerState(panelId, id, { [`${selectedHand.handSide}HandRotation`]: rotation });
  ...
  useEffect(() => {
    const updateArm = (side) => {
      const upperArm = side === 'left' ? leftUpperArmRef.current : rightUpperArmRef.current;
      const lowerArm = side === 'left' ? leftLowerArmRef.current : rightLowerArmRef.current;
  ...
  }, [isSelected, selectedHand, panelId, id, resetHandTransformer, attachHandTransformer]);
  ...
  const renderHead = () => {
    ...
    switch (chosenHead) {
      case "Bow":
        return <Rect {...baseProps} width={bodyWidth / 2} height={bodyHeight * 2} x={-15} />;
      case "Duck":
        return <Arc {...baseProps} outerRadius={15} rotation={180} angle={180} y={10} innerRadius={0} />;
      case "Upright":
      default:
  ...
  const renderHand = (side) => {
    ...
    const isHandSelected = selectedHand && 
                           selectedHand.panelId === panelId && 
                           selectedHand.dancerId === id && 
    ...
    switch (handShape) {
      case "Knee":
        ...
        case "Shoulder":
          return (// wrapped in group so that initial rotation can be set but still change dynamically
            <Group {...baseProps}> 
              <Arc 
                angle={180} 
                innerRadius={0} 
      ...
      case "Overhead":
        return <RegularPolygon {...baseProps} sides={3} radius={7} offsetY={-1} />;
      case "Waist":
      default:
        return <Rect {...baseProps} width={15} height={5} offsetX={7.5} offsetY={2.5} />;
    }
  };
  //Allows clicking of arm to change thickness
  const handleArmClick = useCallback((side, part) => (e) => {
    ...
    updateDancerState(panelId, id, { [`${side}${part}ArmThickness`]: newThickness });
  ...
  return (
    <>
      ...
      {selectedHand && selectedHand.panelId === panelId && selectedHand.dancerId === id && (
        <Transformer
