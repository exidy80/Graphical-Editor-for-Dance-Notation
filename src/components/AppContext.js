...
const createInitialPanel = () => ({
  id: uuidv4(),
  dancers: [ //Dancers have initial values set which are then updated as they are moved etc
    { //Red dancer
      id: uuidv4(), 
      x: 150, 
      y: 40, 
  ...
  handShapes: [//default hand shapes for each hand/dancer
  ...
  export const AppProvider = ({ children }) => {
    ...
    const handleHeadSelection = (shape) => {
        ...
        setPanels(prevPanels => prevPanels.map(panel => {
            ...
            const dancerIndex = panel.dancers.findIndex(dancer => dancer.id === selectedDancer.dancerId);
            if (dancerIndex !== -1) {
        ...
        if (selectedHand) {
          setPanels(prevPanels => prevPanels.map(panel => {
            if (panel.id === selectedHand.panelId) {
              const dancerIndex = panel.dancers.findIndex(dancer => dancer.id === selectedHand.dancerId);
    ...
    const handlePanelSelection = (panelId) => { //updates the ID of selected Panel
        setSelectedPanel(panelId);
      };
    ...
    //Deletes Panel
    const deleteSelectedPanel = useCallback((panelId) => {
    ...
    return (
      <AppContext.Provider value={{
          panels, setPanels, panelSize, movePanel,
          selectedPanel, setSelectedPanel,
          selectedHand, setSelectedHand,
          selectedDancer, setSelectedDancer,
          ...
          addPanel, updateDancerState, updateShapeState,
          serializePanel, deserializePanel, clonePanel, deleteSelectedPanel
      }}>
          {children}
      </AppContext.Provider>
  );
